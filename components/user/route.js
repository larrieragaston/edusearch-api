...
router.get("/users/getUniversityUsers", authenticate, getUniversityUsers);
...
async function createUser(req, res, next) {
	...
	try {
		const user = await Promise.all(
			req.model("User").create({ ...req.body, university: null })
		);
...
async function createUniversityUser(req, res, next) {
	...
	try {
		const [university, user] = await Promise.all([
			...
			req
				.model("User")
				.create({ ...req.body, university: req.params.universityId }),
		]);
...
async function getUniversityUsers(req, res, next) {
	req.logger.info(
		`Finding user for university with id ${req.user.university} using auth token`
	);
	...
	try {
		...
		if (!users) {
			req.logger.verbose("Users not found. Sending Not Found to client");
			return res.status(404).end();
		}
...
async function sendUserPasswordResetByEmail(req, res, next) {
	req.logger.info(
		`Sending password reset email to user with email ${req.params.email}`
	);
	...
	try {
		...
		req.logger.verbose(
			"User password reset email has been sent. Sending 204 to client"
		);
		res.status(204).end();
	} catch (err) {
		next(err);
